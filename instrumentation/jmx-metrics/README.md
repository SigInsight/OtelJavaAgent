...
> Note: this fork only ships the Tomcat rule set. The upstream agent additionally provides
> `activemq`, `camel`, `jetty`, `kafka-broker`, `kafka-connect`, `wildfly`, `hadoop`, and `jvm`
> rule files; they were removed here as part of the agent slimming effort. JVM metrics are
> covered by the [runtime-telemetry](../runtime-telemetry) instrumentation instead.
