...
class SupportedLibrariesAuditorTest {
  ...
  void testPerformAuditWithMissingItems() throws IOException, InterruptedException {
    ...
    try (MockedStatic<FileManager> fileManagerMock = mockStatic(FileManager.class)) {
      ...
      assertThat(result.get()).contains("- Apache CXF JAX-RS");
    }
  }
...
|---------------------------------------------------------------------------------------|------------------------------------------|--------------------------------------------------------|---------------------------------------|
| [Tomcat](https://tomcat.apache.org/)                                              | 10.1.48                                  | OpenJDK 11, 17, 21, 23<br/>OpenJ9 11, 17, 21, 23       | [`ubuntu-latest`], [`windows-latest`] |
""";
  }
